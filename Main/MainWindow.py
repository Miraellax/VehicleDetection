...
from PyQt5.QtCore import Qt, QBuffer, QTimer, QObject, QThread, pyqtSignal, QMutex, QUrl
from PyQt5.QtMultimedia import QSoundEffect

import CustomTitleBar
import ECModel
import BBoxWidget
import create_model_dicts

# init dicts before any code
create_model_dicts.create_dicts()

absolute_path = os.path.dirname(__file__)
logging.basicConfig(filename= absolute_path + '/Main_log.log',
                    filemode='a'
                    )
...
class DetectionWindow(QtWidgets.QWidget):
    ...
    def __init__(self):
        ...
        dict_path = "/resources/data.yaml"
        self.objectClassesDict, self.objectColorsDict = read_class_dict(absolute_path + dict_path)
        self.weights_dict = np.load(absolute_path + '/resources/model_weight_dict.npy', allow_pickle=True).item()
        ...
        self.sound_player.setSource(QUrl.fromLocalFile(absolute_path + "/resources/short_alarm.wav"))
        ...
        # SetWindowDisplayAffinity on top of the flags.
        # print(self.winId())
        # print(int(self.winId()))
        # windll.user32.SetWindowDisplayAffinity(int(self.winId()), hex(2))
    ...
    def update_mask(self):
        ...
        # delete all existing bboxes
        for i in reversed(range(len(self.grabWidget.layout().children()))):
